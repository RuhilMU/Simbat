    ...
    <div class="rounded-lg bg-white p-6 shadow-lg">
        <div class="mb-4 flex items-center">
            ...
            <div class="flex-[1.5] flex flex-col  items-center justify-center">
                <div class="flex items-center">
                    ...
                    <span>{{ $bill->transaction()->code }}</span>
                </div>
                <h1 class="text-center text-2xl font-bold">LAPORAN PENERIMAAN BARANG</h1>
            </div>
            <div class="flex-1 ml-10">
                <p class="text-sm mb-4 text-left">Tanggal: {{ Carbon::parse($bill->created_at)->translatedFormat('j F Y') }}
                </p>
                <h2 class="text-lg font-bold text-left">{{ $bill->transaction()->vendor()->name }}</h2>
                <h2 class="text-sm text-left">{{ $bill->transaction()->vendor()->address }}</h2>
                <h2 class="text-sm text-left">{{ $bill->transaction()->vendor()->phone }}</h2>
            </div>
        ...
        <div class="overflow-hidden rounded-lg bg-white shadow-md mt-8">
            ...
            <h2 type="text" class="text-right mt-6 pe-6 pb-6">Grand total :
                {{ 'Rp ' . number_format($bill->transaction()->outcome, 0, ',', '.') }}</h2>
        </div>
